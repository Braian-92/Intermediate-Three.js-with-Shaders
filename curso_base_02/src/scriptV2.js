...
function crearPunto(parametros, escalas) {
  ...
  box.position.x = restXYZ.x
  box.position.y = restXYZ.y
  box.position.z = restXYZ.z
...
paisesJson.forEach((paisV2Json) => {
  ...
  // if (curvaTest.length < 2) {
    curvaTest.push(new THREE.Vector3(restXYZ.x, restXYZ.y, restXYZ.z))
    curvaTestV2.push({ x: restXYZ.x, y: restXYZ.y, z: restXYZ.z })
  // }
...
const sampleClosedSpline = new THREE.CatmullRomCurve3(curvaTest)
...
const params = {
  spline: 'GrannyKnot',
  scale: 4,
  extrusionSegments: (curvaTest.length + 1), // Incrementa el número de segmentos en el largo 200
  radiusSegments: 20, // Incrementa el número de segmentos en el radio
  closed: true,
  animationView: false,
  lookAhead: false,
  cameraHelper: false
...
const materialC = new THREE.MeshLambertMaterial({ color: 0xff00ff })
const wireframeMaterialC = new THREE.MeshBasicMaterial({
  color: 0xff0000,
  opacity: 1,
  wireframe: true,
  transparent: true
...
const extrudePath = curvaTest

// Usar la curva cerrada como extrusión path para TubeGeometry
tubeGeometry = new THREE.TubeGeometry(
  sampleClosedSpline,
  params.extrusionSegments,
  0.1, // Reducir el radio al 10% del valor actual
  params.radiusSegments, // Usar el nuevo valor de radiusSegments
  params.closed
)

mesh = new THREE.Mesh(tubeGeometry, materialC)
const wireframe = new THREE.Mesh(tubeGeometry, wireframeMaterialC)
mesh.add(wireframe)

group.add( mesh );

function crearTuboDePuntos(puntos, radio) {
  console.log('🚀 ~ crearTuboDePuntos ~ puntos:', puntos)
  // Crear una matriz de vértices para el tubo
  const vertices = []
  puntos.forEach((punto) => {
    vertices.push(punto.x, 0, punto.z) // Añadir cada punto al array de vértices
  })

  // Crear un BufferGeometry y añadir los vértices
  ...
  // Crear un material y un tubo a partir de la geometría
  const material = new THREE.MeshBasicMaterial({ color: 0xff0000 })
  const mesh = new THREE.Mesh(geometry, material)

  // Generar el tubo utilizando la clase THREE.ExtrudeBufferGeometry
  const tuboGeometry = new THREE.ExtrudeBufferGeometry(
    new THREE.Shape(
      vertices.map(
        (_, i) =>
          new THREE.Vector2(
    ...
    { depth: radio, bevelEnabled: false }
  )

  // Establecer la posición y rotación del tubo según los puntos proporcionados
  mesh.geometry = tuboGeometry
  mesh.position.set(0, 0, 0) // Puedes ajustar la posición según tu necesidad
  mesh.rotation.set(0, 0, 0) // Puedes ajustar la rotación según tu necesidad

  return mesh
}

const radio = 0.1 // Radio del tubo
const tubo = crearTuboDePuntos(curvaTestV2, radio)
// group.add(tubo)
